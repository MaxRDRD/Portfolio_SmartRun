...
func (r *userRepository) cacheSet(ctx context.Context, key string, value string, ttl time.Duration) {
	cache.SetString(ctx, r.cache, key, value, ttl)
}

func (r *userRepository) cacheSetWithJitter(ctx context.Context, key string, value interface{}, baseTTL time.Duration) {
	cache.SetJSONWithJitter(ctx, r.cache, key, value, baseTTL)
}

func (r *userRepository) InvalidateUserCache(ctx context.Context, userID int64, email string) {
	cache.DelKey(ctx, r.cache, fmt.Sprintf("user:%d", userID))
	cache.DelKey(ctx, r.cache, fmt.Sprintf("user:email:%d", userID))
	if email != "" {
		cache.DelKey(ctx, r.cache, fmt.Sprintf("user:email:%s", email))
	}
}
...
func (r *userRepository) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
	...
	res, err, _ := r.sf.Do(key, func() (interface{}, error) {
		...
		if _, marshalErr := json.Marshal(&user); marshalErr == nil {
			r.cacheSetWithJitter(ctx, key, &user, 30*time.Minute)
		}
